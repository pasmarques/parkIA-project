...
import { vagas, movimentacoes, validarPlaca, registrarEntrada, registrarSaida, calcularValor } from '@/data/mockData';
...
export default function Movimentacoes() {
  ...
  const handleConfirmarSaida = () => {
    ...
    if (result) {
      toast.success(`Saída registrada. Valor: R$ ${result.valor.toFixed(2)}`);
      setSaidaDialog(null);
      setSaidaPlaca('');
      setActiveMovimentacoes([...movimentacoes]);
    } else {
        ...
        <Tabs defaultValue="entrada" className="w-full">
          ...
          <TabsContent value="entrada" className="mt-6">
            <div className="grid gap-6 lg:grid-cols-2">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>Vagas Disponíveis ({vagasLivres.length})</CardTitle>
                </CardHeader>
                <CardContent>
                  ...
                  {vagasLivres.length > 12 && (
                    <p className="mt-4 text-center text-sm text-muted-foreground">
                      +{vagasLivres.length - 12} vagas disponíveis
                    </p>
                  )}
                </CardContent>
              </Card>
